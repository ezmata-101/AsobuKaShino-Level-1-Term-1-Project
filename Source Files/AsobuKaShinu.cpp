...
void highscore_leaderboard()
{
    ...
    if(playerlevel == 1)
        timebonus=(max(0, (650-time_elapsed))) * 100;
    if(playerlevel == 2)
        timebonus=(max(0, (1050-time_elapsed))) * 100;
    if(playerlevel == 3)
        timebonus=(max(0, (7000-time_elapsed))) * 100;
    lifebonus=life * 500;
...
void timer()
{
    ...
    if((level == 1 || level == 2 || level == 3) && (!completed))
        time_elapsed++;
    if(level==1 || level==2)
    {
        for(int i=danstart[level-1]; i<danend[level-1]; i++)
        {
            ...
            {
                dany[i]+=dangerdy[i];
                if(dany[i]-danorigin[i]>=400||dany[i]<danorigin[i])
                    dangerdy[i]*=-1;
            }
            else if(dancon[i] == -1 || dant[i] == 4)
            {
                danx[i]+=dangerdx[i];
                if(danx[i]-danorigin[i]>=dantravel[i]||danx[i]<danorigin[i])
                    dangerdx[i]*=-1;
            }
        }
        for(int i=linestart[level-1]; i<lineend[level-1]; i++)
        {
            ...
            {
                _line_x[i]+=ldx;
                if(_line_x[i]-_line_origin[i]>=_line_travel[i]||_line_x[i]<=_line_origin[i])
                    ldx*=-1;
                if(_line_t[ballposition]==3&&!jump)
                    ballx+=ldx;
            }
            else if(obst[i]==4)
            {
                _line_y[i]+=ldy;
                if(_line_t[ballposition]==4)
                    bally+=ldy;
                if(_line_y[i]-_line_origin[i]>=_line_travel[i]||_line_y[i]<=_line_origin[i])
                    ldy*=-1;
            }
        }
...
void poxpoy()
{
    if(ballx>=0 && ballx<500)
    {
        pox=0;
        if(level==1)
            strcpy(filename, bmpname[0]);
        if(level==2)
            strcpy(filename, bmpname[4]);
    }
    if(ballx>=500 && ballx<750)
    {
        pox=-350;
        if(level==1)
            strcpy(filename, bmpname[1]);
        if(level==2)
            strcpy(filename, bmpname[5]);
    }
    if(ballx>=750 && ballx<1250)
    {
        pox=-500;
        if(level==1)
            strcpy(filename, bmpname[2]);
        if(level==2)
            strcpy(filename, bmpname[6]);
    }
    if(ballx>=1250&&ballx<=1500)
    {
        pox=-1000;
        if(level==1)
            strcpy(filename, bmpname[3]);
        if(level==2)
            strcpy(filename, bmpname[7]);
    }
    if(ballx>=1500&&ballx<=2200&&level==2)
    {
        pox=-1500;
        if(level==2)
            strcpy(filename, bmpname[8]);
    }
    if(ballx>=2200)
    {
        pox=-2000;
        if(level==2)
            strcpy(filename, bmpname[9]);
    }
}
void newgame()
{
...
void leaderboard()
{
    if(!strcmp(playername, "level1"))
        return;
    if(!strcmp(playername, "level2"))
        return;
    if(!strcmp(playername, "level3"))
        return;
...
void iLevel()
{
    if(level==1 || level==2)
    {
        ...
        for(int i=danstart[level-1]; i<danend[level-1]; i++)
        {
            if(dant[i]==1&&!explosion&&deathtimer)
                iShowBMP2(pox+danx[i], poy+dany[i], "Games//grenade.bmp", 0xffffff);
            if(dant[i]==3||dant[i]==4)
                iShowBMP2(pox+danx[i], poy+dany[i], "Games//bullet.bmp", 0xffffff);
            if(dant[i]==101 && dancon[i])
                iShowBMP2(pox+danx[i], poy+dany[i], "Games//life.bmp", 0xffffff);
        }
        for(int i=obsstart[level-1]; i<obsend[level-1]; i++)
        {
            if(obst[i] == 1)
                iShowBMP(pox+obsx[i], obsy[i], "Games//box.bmp");
            else if(obst[i]==3)
                iFilledRectangle(pox+obsx[i],poy+obsy[i],100,5);
        }
...
void iMenu()
{
    ...
    if(!ft)
        iShowBMP2(455, 170, "Games//Menu//newgame.bmp", 0xffffff);
    else
        iShowBMP2(455, 170, "Games//Menu//howto.bmp", 0xffffff);
    iShowBMP2(495, 170, "Games//Menu//info.bmp", 0xffffff);
    if(sound>0)
        iShowBMP2(533, 170, "Games//Menu//sound1.bmp", 0xffffff);
    else
        iShowBMP2(533, 170, "Games//Menu//sound0.bmp", 0xffffff);
...
{
    if(!namelen)
        iShowBMP(0, 0, "Games//profile//back2.bmp");
    else
    {
...
void iLeaderBoard()
{
    ...
    for(int i=2; i<=leaderboardnum; i++)
    {
        ...
        if(!strcmp(playername, leaderboardname[i-1]))
            iSetColor(255, 0, 0);
        else
            iSetColor(255, 255, 0);
